...
# A more programmatic way would be something like this
activities <- activities %>% 
  mutate(
    across(Distance, 
           ~ifelse(`Activity Type` == "Swim", .x / 1093.61329834, .x)),
    `Elevation Gain` = ifelse(is.na(`Elevation Gain`), 0, `Elevation Gain`)
  )
...
# do the above inside mutate instead
activities <- activities %>% 
...
gpxdata <- slice(gpxdata, -c(63744, 63745, 63746)) # What's wrong with these rows? Would be better to identify them based on some condition
...
# Instead use teh column name
activities_tight <- activities_tight %>% 
  select(-activity_id)
...
# again, better to use teh condition, e.g., 
# activities2 %>% filter(activity_type != "E-Bike" & activity_type != "Swim)
```
...
# I would instead do a line graph showing something like total number of each 
# activities each month. Even that is a little messy though, so I'd facet_wrap it
# activities3 %>% 
#   mutate(date = round_date(activity_date, "month")) %>% 
#   count(date, activity_type) %>% 
#   ggplot(aes(date, n)) +
#   geom_line(aes(color = activity_type)) +
